...
void ShapeDiamond::draw(windows_console::image & anImage, windows_console::brightness_type const & aBrightness, windows_console::color_type const & aColor) const {
	...
	anImage << line(static_cast<int>(mDiamond[mDiamond.size() - 1].x()), static_cast<int>(mDiamond[mDiamond.size() - 1].y()), static_cast<int>(mDiamond[0].x()), static_cast<int>(mDiamond[0].y()));
}
